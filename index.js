...
</li>`).join(""),P=async(e,s)=>{try{Te();const t=await Me(e,s);Xe(),C()?o.petsList.insertAdjacentHTML("beforeend",K(t.data.animals)):o.petsList.innerHTML=K(t.data.animals),k=t.data.totalItems,o.petsList.querySelectorAll(".pets-item").forEach((i,r)=>{const c=t.data.animals[r];c&&(i.dataset.description=c.description||"",i.dataset.health=c.healthStatus||"",i.dataset.behavior=c.behavior||"")})}catch(t){ie(t.message)}},ne=()=>o.petsList.innerHTML="",Pe=e=>{document.querySelectorAll(".pets-ctgs-btn").forEach(t=>{t.classList.remove("active"),t.setAttribute("aria-pressed","false")}),e.classList.add("active"),e.setAttribute("aria-pressed","true")},Ae=async e=>{e.target.nodeName==="BUTTON"&&(_(),V(),Pe(e.target),x=e.target.dataset.id==="all"?void 0:e.target.dataset.id,d=1,ne(),await P(d,x),C()&&k>w?R():C()||N())};o.ctgsList.addEventListener("click",Ae);const Ie=async()=>{d++,await P(d,x),d*w>=k&&_()};o.petsLoadBtn.addEventListener("click",Ie);const _=()=>{o.petsLoadBtn.style.display="none"},R=()=>{o.petsLoadBtn.style.display="block"},Xe=()=>{o.petsLoader.style.display="none"},Te=()=>{o.petsLoader.style.display="block"},V=()=>{o.petsPagination.style.display="none"},Oe=()=>{const e=ae();e!==B&&(ne(),_(),V(),B=e,w=oe(),d=1,P(d,x),e==="mobile"&&d*w<k?R():e!=="mobile"&&N())};window.addEventListener("resize",Oe);const De=async()=>{await Ce(),await P(d),C()&&k>w?R():C()||N()};De();function je(){return`
